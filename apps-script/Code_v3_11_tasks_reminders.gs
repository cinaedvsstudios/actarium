...
function doGet(e) {
  try {
    ...
    if (action === 'bootstrap') return json_({
      success: true,
    ...
    return json_({ success: false, error: message_(error) });
  }
}
...
function saveReminder_(reminder) {
  ...
  const alarmEnabled = yes_(reminder.alarmEnabled) || yes_(reminder.alarm_enabled);
